...
### Inital Stage of Development of the Quoridor Env ###
### Has since been refactored ###

class QuoridorOldEnv(gym.Env):
    ...
    def __init__(self,seed=None):
        ...
        self.action_space = spaces.Discrete(12+(self.array_height-1)*(self.array_width-1)*2)
        self.observation_space = spaces.Dict({
            "board": spaces.Box(0,2,[self.array_width,self.array_height]), #array with shape (9,9) with numbers ranging from 0-2
            "walls": spaces.Box(0,2,[self.array_width-1, self.array_height-1]), #array with shape (8,8) with numbers ranging from 0-2
            "player_walls": spaces.Discrete(6), #number of walls is between 0-5
            "opponent_walls": spaces.Discrete(6), #number of walls is between 0-5
            "turn": spaces.Discrete(2) #turn number is between 0-1
            }
        )
        self.reset()
    ...
    def set_state(self,state):
        ...
        self.player_walls = state["player_walls"] 
        self.opponent_walls = state["opponent_walls"] 
        self.turn = state["turn"]

    def step(self,action):

        self.last_state = self._get_game_state()
        valid, newstate = self._valid_action(self._get_game_state(),self.turn,action)
        if valid:
            self.set_state(newstate)
            if self._is_over(newstate):
                return self.state_to_array(self._get_game_state()), self._get_reward(self._get_game_state(),self.last_state), self._is_over(newstate), {"current":self._get_game_state(),"last":self.last_state}
            else:    
                return self.state_to_array(self._get_game_state()), self._get_reward(self._get_game_state(),self.last_state), self._is_over(newstate), {"current":self._get_game_state(),"last":self.last_state}
        else:
            return self.state_to_array(self._get_game_state()), -15, self._is_over(self._get_game_state()), {"current":self._get_game_state(),"last":self._get_game_state()}

    def state_to_array(self,state):
        state = self.copy_state(state)
        for statekey in state.keys():
            state[statekey] = np.array(state[statekey])
        return state
    ...
    def render(self, mode="rgbarray"):
        ...
        """
        return render_state(self._get_game_state(),mode=mode)

    def render_state(self,state,mode="rgbarray"):
        pass
    ...
    def _valid_action(self,state,player,action):
        ...
        movement,placement = self.action_to_movementplacement(action)

        if placement is None and movement is not None:

            # Check to see if movement is to a valid position

            mx, my = movement
            playerposx, playerposy = self._find_player_pos(state,player)
            newposx, newposy = playerposx + mx, playerposy+ my
            if (newposx, newposy) in self._player_movement(state,player):
                newstate['board'][playerposy][playerposx] = 0
                newstate['board'][newposy][newposx] = player
                self.next_turn(newstate)
                return True, newstate
            else:
                print(f"Movement failed from {playerposx,playerposy} to {newposx,newposy}")
                return False, state

        elif placement is not None and movement is None:
            x, y, d = placement
            if self._allowed_placement(state,(x,y),d):
                if player == 1 and state['player_walls'] <= 0:
                    return False, state
                elif player == 2 and state['opponent_walls'] <= 0:
                    return False, state
                else:
                    newstate['walls'][y][x] = d
                    if self._no_maze(newstate):
                        return False, state
                    else:
                        self.next_turn(newstate)
                        if player == 1:
                            newstate['player_walls'] -= 1
                        else:
                            newstate['opponent_walls'] -= 1
                        return True, newstate
        ...
        if walldir in walls_dir:
            if walldir == WallDir['HORIZONTAL']:
                ...
                if walls_dir[MovementIndex['UP'].value] == walldir or walls_dir[MovementIndex['DOWN'].value] == walldir:
                    return False
        elif state['walls'][wallpos[1]][wallpos[0]] != 0:
            return False
    ...
    def _is_over(self,state):
        return self._no_maze(state) or self.loc_at_finish(state,1) or self.loc_at_finish(state,2)

    def loc_at_finish(self,state,player):
        if player == 1:
            return self._find_player_pos(state,player) in self.player_endpos_list
        else:
            return self._find_player_pos(state,player) in self.opponent_endpos_list
